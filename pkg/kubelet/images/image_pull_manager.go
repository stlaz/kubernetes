...
// successful pulls.
// It tracks the credentials used with each successfull pull in order to be able
...
func (f *FileBasedImagePullManager) PruneUnknownRecords(imageList []string, until time.Time) {
	...
	err := filepath.WalkDir(f.pulledDir, func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			klog.V(5).Infof("skipping %q - it is a directory", path)
			return nil
		}

		fileContent, err := os.ReadFile(path)
		if err != nil {
			klog.V(5).Infof("skipping %q - failed to read: %v", path, err)
			return nil
		}

		pullRecord := imagemanagerv1alpha1.ImagePulledRecord{}
		if err := json.Unmarshal(fileContent, &pullRecord); err != nil {
			klog.V(5).Infof("skipping %q - failed to deserialize: %v", path, err)
			return nil
		}
	...
	if err != nil {
		klog.Errorf("failed to garbage collect ImagePulledRecord files: %v", err)
		return
	}
	...
	for _, ir := range imageRecords {
		...
		if err := os.Remove(recordToRemovePath); err != nil {
			klog.Errorf("failed to remove %q: %v", recordToRemovePath, err)
		}
	}
...
func (f *FileBasedImagePullManager) startupHousekeeping(ctx context.Context) {
	...
	err := filepath.WalkDir(f.pullingDir, func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			klog.Warningf("%q is a directory. Skipping.", path)
			return nil
		}

		fileContent, err := os.ReadFile(path)
		if err != nil {
			klog.Errorf("skipping %q, failed to read: %v", path, err)
			return nil
		}

		var intent imagemanagerv1alpha1.ImagePullIntent
		if err := json.Unmarshal(fileContent, &intent); err != nil {
			klog.Errorf("skipping %q, failed to deserialize to ImagePullIntent: %v", path, err)
			return nil
		}
	...
	if err != nil {
		klog.Errorf("there was an error walking the %q directory: %v", f.pullingDir, err)
		return
	}
	...
	for _, image := range images {
		...
		f.writePulledRecord(imageName, image.ID, nil)
		removePath := inFlightPulls[imageName]
		if err := os.Remove(removePath); err != nil {
			klog.Warningf("failed to remove image pull intent file %q: %v", removePath, err)
		}
