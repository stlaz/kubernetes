...
import (
	...
	imagemanagev1alphav1 "k8s.io/kubernetes/pkg/kubelet/apis/config/imagemanager/v1alphav1"
)
...
type ImagePullManager interface {
	...
	//
	// `image` is the content of the pod's container `image` field.
	RecordImagePulled(image, imageRef string, credentials *imagemanagev1alphav1.ImagePullCredentials)
	...
	//
	// `image` is the content of the pod's container `image` field.
	MustAttemptImagePull(image, imageRef string, credentials []imagemanagev1alphav1.ImagePullSecret) bool
