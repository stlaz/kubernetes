...
func (pip *parallelImagePuller) pullImage(ctx context.Context, spec kubecontainer.ImageSpec, credentials []credentialprovider.TrackedAuthConfig, pullChan chan<- pullResult, podSandboxConfig *runtimeapi.PodSandboxConfig) {
	go func() {
		...
		startTime := time.Now()
		imageRef, imageID, creds, err := pip.imageService.PullImage(ctx, spec, credentials, podSandboxConfig)
		var size uint64
		if err == nil && imageID != "" {
...
func (sip *serialImagePuller) processImagePullRequests() {
	for pullRequest := range sip.pullRequests {
		startTime := time.Now()
		imageRef, imageID, creds, err := sip.imageService.PullImage(pullRequest.ctx, pullRequest.spec, pullRequest.credentials, pullRequest.podSandboxConfig)
		var size uint64
		if err == nil && imageID != "" {
			// Getting the image size with best effort, ignoring the error.
